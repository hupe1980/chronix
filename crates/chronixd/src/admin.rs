...
// ── Chaos injection admin API ─────────────────────────────────────────

#[cfg(feature = "chaos")]
/// Request body for `POST /api/v1/admin/chaos/inject`.
#[derive(Debug, Deserialize)]
pub struct ChaosInjectRequest {
    /// The fault type to inject.
    pub fault: chronix_chaos::Fault,
    /// How long the fault remains active (in seconds).
...
#[cfg(feature = "chaos")]
/// Response for a chaos injection.
#[derive(Debug, Serialize)]
pub struct ChaosInjectResponse {
    /// The assigned injection ID.
...
#[cfg(feature = "chaos")]
/// Serializable injection info.
#[derive(Debug, Serialize)]
pub struct ChaosInjectionInfo {
    /// Injection ID, as returned by `chaos_inject_handler`.
    pub id: u64,
    /// Fault kind.
    pub fault: String,
    /// Human-readable description of what the fault does.
    pub description: String,
    /// Seconds left before the injection expires on its own.
    pub remaining_secs: f64,
}

#[cfg(feature = "chaos")]
/// `POST /api/v1/admin/chaos/inject` — inject a fault.
pub async fn chaos_inject_handler(
    State(state): State<AppState>,
    Json(body): Json<ChaosInjectRequest>,
) -> Result<impl IntoResponse, ServerError> {
    let agent = state
        .chaos_agent
        .as_ref()
        .ok_or_else(|| ServerError::BadRequest("chaos agent not enabled".into()))?;

    let config = chronix_chaos::FaultConfig {
        fault: body.fault.clone(),
    ...
    // Store the guard persistently instead of leaking it with mem::forget.
    // The guard's Drop impl would clear the injection; storing it keeps the
    // injection alive without leaking the Arc refcount.
    state.chaos_guards.lock().push(guard);

    info!(
        injection_id = id,
        fault = %body.fault,
        duration_secs = body.duration_secs,
        "audit: chaos fault injected"
    );

    Ok((
        StatusCode::CREATED,
        Json(ChaosInjectResponse {
            injection_id: id,
            fault: body.fault.to_string(),
            duration_secs: body.duration_secs,
        }),
    ))
}
...
pub async fn chaos_list_handler(
    State(state): State<AppState>,
) -> Result<Json<Vec<ChaosInjectionInfo>>, ServerError> {
    let agent = state
        .chaos_agent
        .as_ref()
        .ok_or_else(|| ServerError::BadRequest("chaos agent not enabled".into()))?;

    let injections: Vec<ChaosInjectionInfo> = agent
        .active_injections()
        .into_iter()
        .map(|info| ChaosInjectionInfo {
            id: info.id,
            fault: info.fault.to_string(),
            description: info.description.clone(),
            remaining_secs: info.remaining().as_secs_f64(),
        })
        .collect();
...
pub async fn chaos_clear_handler(
    State(state): State<AppState>,
    Path(id): Path<u64>,
) -> Result<impl IntoResponse, ServerError> {
    let agent = state
    ...
    // Remove the guard from persistent storage.
    state.chaos_guards.lock().retain(|g| g.id() != id);

    info!(injection_id = id, "audit: chaos injection cleared");
...
pub async fn chaos_clear_all_handler(
    State(state): State<AppState>,
) -> Result<impl IntoResponse, ServerError> {
    let agent = state
        .chaos_agent
    ...
    // Remove all stored guards.
    state.chaos_guards.lock().clear();

    info!("audit: all chaos injections cleared");
...
mod tests {
    ...
    #[cfg(feature = "chaos")]
    #[test]
    fn chaos_inject_request_deserialization() {
        let json = r#"{"fault": "DiskFull", "duration_secs": 30, "description": "test"}"#;
        let req: ChaosInjectRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.duration_secs, 30);
        assert_eq!(req.description, "test");
        assert_eq!(req.fault, chronix_chaos::Fault::DiskFull);
    }

    #[cfg(feature = "chaos")]
    #[test]
    fn chaos_inject_request_latency_spike() {
        let json = r#"{"fault": {"LatencySpike": {"delay": {"secs": 1, "nanos": 0}}}, "duration_secs": 60}"#;
        let req: ChaosInjectRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.duration_secs, 60);
        match req.fault {
            chronix_chaos::Fault::LatencySpike { delay } => {
                assert_eq!(delay, std::time::Duration::from_secs(1));
            }
            _ => panic!("expected LatencySpike"),
        }
    }

    #[cfg(feature = "chaos")]
    #[test]
    fn chaos_injection_info_serialization() {
        let info = ChaosInjectionInfo {
            id: 42,
            fault: "DiskFull".into(),
            description: "test disk full".into(),
            remaining_secs: 25.5,
        };
        let json = serde_json::to_value(&info).unwrap();
        assert_eq!(json["id"], 42);
        assert_eq!(json["fault"], "DiskFull");
        assert_eq!(json["description"], "test disk full");
    }

    #[cfg(feature = "chaos")]
    #[test]
    fn chaos_inject_response_serialization() {
        let resp = ChaosInjectResponse {
            injection_id: 1,
            fault: "SlowDisk".into(),
            duration_secs: 10,
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"injection_id\":1"));
        assert!(json.contains("SlowDisk"));
    }
