...
- **Security depth** — Cedar policies (a formally verified engine, with the
  ...
  are off by default, which is most of the dependency graph an embedded build does not
  carry.
...
for the privilege — **roughly doubling the dependency graph**. `chronixd` takes all three
...
| `chronix-streaming` | `arrow` | off | CDC events as Arrow `RecordBatch`es — adds `arrow` |
